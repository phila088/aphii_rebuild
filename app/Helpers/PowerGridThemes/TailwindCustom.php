...
class TailwindCustom extends TailwindBase
{
    ...
    public function table(): Table
    {
        return Theme::table('min-w-full dark:bg-slate-800')
            ->div('rounded-t-md relative border-x border-t border-pg-primary-200 dark:bg-pg-primary-700 dark:border-pg-primary-600')
            ->thead('shadow-md rounded-t-md bg-pg-primary-200 dark:bg-gray-800')
            ->thAction('!font-bold text-center')
            ->tdAction('')
            ->tr('')
            ->trFilters('text-xs bg-white shadow-sm dark:bg-pg-primary-800')
            ->th('font-semibold px-2 pr-4 py-3 text-center text-xs font-semibold text-pg-primary-700 tracking-wider whitespace-nowrap dark:text-pg-primary-300')
            ->tbody('text-pg-primary-800')
            ->trBody('border-b border-pg-primary-100 dark:border-pg-primary-600 hover:bg-slate-100 dark:hover:bg-pg-primary-600')
            ->tdBody('pl-[19px] px-3 py-2 text-xs text-center whitespace-nowrap dark:text-pg-primary-200')
            ->tdBodyEmpty('px-3 py-2 whitespace-nowrap dark:text-pg-primary-200')
            ->trBodyClassTotalColumns('')
            ->tdBodyTotalColumns('px-3 py-2 whitespace-nowrap dark:text-pg-primary-200 text-xs text-pg-primary-600 text-right space-y-2');
    }
    ...
    public function footer(): Footer
    {
        return Theme::footer()
            ...
            ->select('text-xs block appearance-none bg-pg-primary-50 border border-pg-primary-300 text-pg-primary-700 py-2 px-3 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-pg-primary-500 dark:bg-pg-primary-700 dark:text-pg-primary-200 dark:placeholder-pg-primary-200 dark:border-pg-primary-500');
    }
    ...
    public function checkbox(): Checkbox
    {
        return Theme::checkbox()
            ...
            ->input('shrink-0 mt-0.5 border-gray-200 rounded-sm text-blue-600 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-gray-800 dark:border-gray-700 dark:checked:bg-blue-500 dark:checked:border-blue-500 dark:focus:ring-offset-gray-800');
    }
    ...
    public function filterSelect(): FilterSelect
    {
        return Theme::filterSelect()
            ...
            ->base('min-w-[9.5rem] text-xs')
            ->select('text-xs appearance-none block bg-white border border-pg-primary-300 text-pg-primary-700 py-2 px-3 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-pg-primary-500 w-full dark:bg-pg-primary-800 dark:text-pg-primary-200 dark:placeholder-pg-primary-300 dark:border-pg-primary-600');
    }
